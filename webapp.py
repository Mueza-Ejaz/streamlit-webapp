    ...
    for file in uploaded_files:
        ...
        try:
            if file_ext == ".csv":
                ...
                df = pd.read_excel(file, engine=None)  # Auto-detect engine
            else:
        ...
        # Display total rows for debugging missing data
        st.write(f"**Total Rows in {file.name}:** {df.shape[0]}")

        # File preview - Full Data
        st.write(f"### Preview of {file.name}")
        st.dataframe(df)  # Shows full data
